...
class PoseCommander(Node):
    def __init__(self, node_name: str) -> None:
        ...
        self.callback_group2.add_entity(self.right_pose_commander_server)

        # Subscriber to new robot position
        #self.target_pose_sub = self.create_subscription(Pose, "/target_pose_right", self.plan_new_target, 1)
        #self.callback_group2.add_entity(self.target_pose_sub)

        # Subscriber to new robot angles
        #self.target_joints_sub = self.create_subscription(LBRPositionCommand, "/target_joints_right", self.plan_new_joint_target, 1)
    ...
    # 3. CHECK :) - Import new robot model, and make a right specific version of this program
